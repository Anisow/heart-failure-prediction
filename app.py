...
def user_input_features():
    ...
    sex = st.selectbox('Sex', options=list(sex_options.keys()))  # Changed to 'Female' and 'Male'
    chest_pain_type = st.selectbox('Chest Pain Type', options=list(chest_pain_options.keys()))  # Changed to names
    ...
    fasting_blood_sugar = st.selectbox('Fasting Blood Sugar > 120 mg/dl', options=list(fasting_blood_sugar_options.keys()))  # Changed to 'No' and 'Yes'
    resting_ecg = st.selectbox('Resting ECG', options=list(resting_ecg_options.keys()))  # Changed to appropriate names
    max_heart_rate = st.number_input('Max Heart Rate Achieved', min_value=0, max_value=250, value=150)
    exercise_angina = st.selectbox('Exercise Induced Angina', options=list(exercise_angina_options.keys()))  # Changed to 'No' and 'Yes'
    oldpeak = st.number_input('Oldpeak', min_value=0.0, max_value=10.0, value=1.0)
    st_slope = st.selectbox('ST Slope', options=list(st_slope_options.keys()))  # Changed to names
    
    data = {
        'age': age,
        'sex': sex_options[sex],  # Map to numeric
        'chest pain type': chest_pain_options[chest_pain_type],  # Map to numeric
        'resting bp s': resting_bp_s,
        'cholesterol': cholesterol,
        'fasting blood sugar': fasting_blood_sugar_options[fasting_blood_sugar],  # Map to numeric
        'resting ecg': resting_ecg_options[resting_ecg],  # Map to numeric
        'max heart rate': max_heart_rate,
        'exercise angina': exercise_angina_options[exercise_angina],  # Map to numeric
        'oldpeak': oldpeak,
        'ST slope': st_slope_options[st_slope]  # Map to numeric
    }
